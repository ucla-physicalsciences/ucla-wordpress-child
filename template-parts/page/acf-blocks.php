...
if( get_row_layout() == 'c-cta-block' ):
  $c_cta_block_heading = get_sub_field('c-cta-block-heading');
  $c_cta_block_summary = get_sub_field('c-cta-block-summary');
  
  echo  '<div class="c-section">';
  ?>  
  <?php if ($c_cta_block_heading): ?>
    <h2><?php echo $c_cta_block_heading; ?></h2>
    <?php endif; ?>
    <?php if ($c_cta_block_summary): ?>
    <p><?php echo $c_cta_block_summary; ?></p>
    <?php endif; ?>
    <?php
    $c_cta_block_list = esc_html(get_sub_field('c-cta-block-list'));
  
    // links repeater starts here 
    if( have_rows('c-cta-block-list') ): // check if the nested repeater field has rows of data

      
      echo '<ul class="ucla-ps-c-button-list">';
      while (have_rows('c-cta-block-list')): the_row();
      
        $c_cta_block_text = get_sub_field('c-cta-block-text');
        $c_cta_block_link = get_sub_field('c-cta-block-link'); 
      
      if ($c_cta_block_list): ?>
        <li><a class="ucla-btn ucla-btn--primary" href="<?php echo esc_url($c_cta_block_link); ?>"><?php echo $c_cta_block_text; ?></a></li>
      <?php endif;
